...
class WinSolution: Gtk.ApplicationWindow
	...
	def dic_stats1 (datos_1:dict of(string,string))
		...
		solution_text_final = ""
		lista_1 = new list of string		
		
		spaces(1, datos_1)		
		spaces(2, datos_1)

		col2: int = lista_1.size/2		
		for var i = 0 to ((lista_1.size/2)-1)
			solution_text_final += lista_1.get(i) + lista_1.get(col2+i) + "\n"		
	...
	def dic_stats2 (datos_1:dict of(string,string), datos_2:dict of(string,string))
		...
		col2: int = lista_1.size/3		
		col3: int = col2 * 2
		for var i = 0 to ((lista_1.size/3)-1)
			solution_text_final += lista_1.get(i) + lista_1.get(col2+i) + lista_1.get(col3+i) + "\n"
